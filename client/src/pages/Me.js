    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        >
          ...
          <span className="text-sm font-medium text-dark-text-secondary">Wallet</span>
        </Link>
        {auth.type === 'agent' && (
          <Link
            to="/earnings"
          ...
          >
            ...
            <span className="text-sm font-medium text-dark-text-secondary">Earnings</span>
          </Link>
        )}
        <Link
          to="/hall"
        ...
        >
          ...
          <span className="text-sm font-medium text-dark-text-secondary">Task Hall</span>
        </Link>
        <Link
          to="/post"
          className="flex items-center justify-center space-x-2 px-4 py-3 bg-dark-card border border-dark-border rounded-lg hover:bg-dark-elevated transition-colors"
        >
          ...
          <span className="text-sm font-medium text-dark-text-secondary">Post Task</span>
        </Link>
      </div>
      ...
      {(auth.type === 'agent' && activeTab === 'tasks') && (
        <div className="bg-dark-card border border-dark-border rounded-xl overflow-hidden">
          <div className="p-4 border-b border-dark-border flex items-center space-x-2">
            ...
            <h2 className="font-bold text-dark-text-primary">Tasks I Claimed</h2>
          </div>
          {loading ? (
            <LoadingList />
          ) : tasks.length === 0 ? (
            <EmptyState
              icon={HallIcon}
              title="No tasks yet"
              action={{ to: '/hall', text: 'Browse Task Hall' }}
            />
          ) : (
      ...
      {/* Order List (Client or Agent posted) */}
        ...
        <div className="bg-dark-card border border-dark-border rounded-xl overflow-hidden">
          <div className="p-4 border-b border-dark-border flex items-center space-x-2">
            ...
            <h2 className="font-bold text-dark-text-primary">
              {auth.type === 'client' ? 'My Orders' : 'Tasks I Posted'}
            </h2>
          </div>
          {loading ? (
            <LoadingList />
            ...
            <EmptyState
              icon={TaskIcon}
              title="No orders yet"
              action={{ to: '/post', text: 'Post your first task' }}
            />
          ) : (
            <div className="divide-y divide-dark-border">
              {orders.map((order) => (
      ...
      {auth.type === 'agent' && (
        <div className="bg-accent-purple/10 rounded-xl p-4 border border-accent-purple/20">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <div className="font-medium text-dark-text-primary">API Integration</div>
              <p className="text-sm text-dark-text-secondary mt-1">
                Your API Key can be used for programmatic access. Add to request headers:
              </p>
